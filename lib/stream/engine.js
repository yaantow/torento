  ...
  for (const item of items) {
    ...
    try {
      ...
      if (item.fileIndex === null || item.fileIndex === undefined) {
        const cacheFiles = cacheManager.getCacheFiles();
      ...
      queue.markError(item.infoHash, e.message);
    }
  }
...
async function getTorrent(magnet, opts) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      ...
      queue.markError(infoHash, 'No metadata peers after 60s');
    ...
    torrent.on('ready', () => {
      ...
      progressInterval = setInterval(() => {
        ...
        for (let fi = 0; fi < torrent.files.length; fi++) {
          ...
          if (cachedFiles.has(file.name)) continue;
