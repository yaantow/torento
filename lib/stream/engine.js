...
async function getTorrent(magnet, opts) {
  ...
  if (existing) {
    if (fileToKeep !== null && fileToKeep !== undefined && existing.files) {
      try {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      ...
      torrent.destroy();
      queue.markError(infoHash, 'No peers found after 15s');
    ...
    torrent.on('ready', () => {
      ...
      progressInterval = setInterval(() => {
        ...
        queue.updateProgress(infoHash, pct);
        if (torrent.progress >= 1) { clearInterval(progressInterval); progressInterval = null; }
      }, 5000);
    ...
    torrent.on('error', (err) => {
      if (progressInterval) clearInterval(progressInterval);
      clearTimeout(timeout);
      queue.markError(infoHash, err.message);
      reject(err);
    });
    torrent.on('done', () => {
      if (progressInterval) { clearInterval(progressInterval); progressInterval = null; }
      queue.markCached(infoHash);
